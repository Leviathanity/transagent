...
export async function stageTranslate(
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    const result = await promptAndGetText(session, `翻译以下 Markdown 内容：\n\n${block.text}`);
    translationMap.set(block.id, result || block.text);
    console.log(`  [${i + 1}/${blocks.length}] ${block.id} done`);
  }
  await session.dispose();

  const assembled = assembleFromSeparatedBlocks(blocks, (block) => translationMap.get(block.id) ?? block.text);
